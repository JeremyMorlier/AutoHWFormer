# Adapted from Torchvision vision transformer
import math
...
class SuperNetEncoder(nn.Module):
...
class SuperNet(nn.Module):
    ...
    def __init__(
        ...
        # Add a class token
        self.class_token = nn.Parameter(torch.zeros(1, 1, hidden_dims[0]))
        seq_length += 1

        self.encoder = SuperNetEncoder(
            seq_length,
            num_layers,
            num_headss,
            hidden_dims,
        ...
        if representation_size is None:
            heads_layers["head"] = nn.Linear(hidden_dims[-1], num_classes)
        else:
            heads_layers["pre_logits"] = nn.Linear(hidden_dims[-1], representation_size)
    ...
    def _process_input(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.reshape(n, self.hidden_dims[0], n_h * n_w)
