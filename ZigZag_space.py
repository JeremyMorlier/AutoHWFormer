...
def zigzag_performance(config) :
    ...
    torch_model = SuperNet(224, 8, model_config["layer_num"], model_config["num_heads"], hidden_dims=model_config["embed_dim"], mlp_ratios=model_config["mlp_ratio"])
    ...
    for opt in opts :
        result[opt] = {}
        energy, latency, cme = api.get_hardware_performance_zigzag(workload=inferred_path, accelerator=accelerator_path, mapping=mapping_path, opt=opt)
